...
> Last updated: **2026-06-19** (animation performance overhaul). Scope of this section: what is actually shipped under `rebuild/`, mapped back to the audit. Companion to [`MODERNIZATION_PLAN.md` §1.5](MODERNIZATION_PLAN.md#15-implementation-progress-live). The canonical live status is [`rebuild/docs/PHASE_1A_STATUS.md`](rebuild/docs/PHASE_1A_STATUS.md).
...
- `mount()` (deferred to `requestIdleCallback`) splits work across **two engines**: the one-shot reveals run on a single shared `IntersectionObserver` + CSS transitions (`registerReveals`), while scroll-scrubbed and draw/count effects keep GSAP + ScrollTrigger. This swap cut ScrollTriggers ~85 → ~21 and persistent `will-change` layers ~72 → ~7 — see the perf note in §0.2.

### 0.2 Primitives — current vs. target

| Primitive            | DOM hook                                     | Implements legacy                                                              | Status                                                                                                                                                                                                                    |
| -------------------- | -------------------------------------------- | ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CSS intro keyframes  | `data-anim="intro-up\|down\|fade"`           | `slide-up-intro`, `fade-up-intro`, `fade-down-intro`, `hero_img-intro`         | ✅ Live (LCP un-gate, 2026-07-08, #43). **Pure CSS keyframes** (`global.css`) that fire at parse — no longer a GSAP boot timeline (`mountIntro` removed). Above-the-fold hero paints at first paint. Stagger via inline `--intro-delay`; `animation-fill-mode: both`. |
| `registerReveals`    | `data-anim="slide-up\|fade-up\|fade-in\|rotate-in\|cover-up"` | `slide-up*`, `fade-up*`, `fade-in*`, `rotate-in*`, `cover-up*`  | ✅ Live (perf overhaul, 2026-06-19). **One shared `IntersectionObserver` + CSS transitions** — replaced the four separate GSAP reveal primitives (`registerSlideUp/FadeUp/RotateIn/CoverUp`, now removed). Flips `data-anim-shown`; CSS does the transform/opacity/clip-path on the compositor. `data-anim-delay` → `transition-delay`. **Known gap (untracked):** legacy's per-line `slide-up__lines` variant (display headings like `#team` "Meet our talents & experts", `#contact` "write us / visit us", via `SectionTitle`) reveals line-by-line; the rebuild animates these headings whole-block. Distinct from `slide-up2__lines`/#10 (different class — a `slide-up` split, not `cover-transp`). Fidelity miss, not yet ticketed. |
| `registerCoverTransp` | `data-anim="cover-transp"`                  | `slide-up2__lines cover-transp`, `fade-up__text__lines cover-transp`           | ✅ Live (#9, 2026-07-09). GSAP **SplitText** `type:"lines", mask:"lines"` splits the text; each line wipes up from inside its clip mask, staggered top→bottom on scroll (`gsap.from` yPercent:100, 0.6s, 0.1s stagger, power3.out). `autoSplit` re-splits on resize/font-load. `aria:"none"` keeps the split spans natively in the a11y tree. Used by `About` + `Featured` body copy (#9) and `Team` + `Contact` intro copy (#10 — legacy `slide-up2__lines`, same effect, switched from whole-block `slide-up`). Reusable line splitter also for #64. |
| `registerParallaxBg` | `data-anim="parallax-bg"`                    | `.parallax` + `.parallax__el` background-position scroll                       | ✅ Live (GSAP scrub). Used by `VideoStrip`. Strength via `data-anim-strength`. Scoped `will-change: transform`.                                                                                                           |
| `registerParallaxY`  | `data-anim="parallax-y"`                     | (new — fills the missing vertical parallax slot)                               | ✅ Live (GSAP scrub). Strength via `data-anim-strength` (px), direction via `data-anim-direction="up\|down"`. Used by `Featured` images + numerals. Scoped `will-change: transform`.                                      |
| `registerOdometer`   | `data-anim="odometer"`                       | `js-odometer--simple`                                                          | ✅ Live (GSAP). Counts to `data-anim-end`; skips redundant text writes. The "999999 → ~1 Million+" string-swap behaviour is **deferred** (Phase 1B).                                                                      |
| `registerSvgDraw`    | `data-anim="svg-draw"`                       | `svg-draw`, `svg-draw__el`, `svg-draw--filled`                                 | ✅ Live (GSAP **DrawSVG**, free since 2025). Draws Service icons; opt-in `data-anim-fill` draws the outline then fades the fill for filled glyphs — Featured numerals (`NumeralGlyph.astro`) and the Testimonials quote watermark (`QuoteGlyph.astro`, done 2026-06-19). |
| `registerCoverDR`    | `data-anim="cover-d-r-img"`                  | `cover-d-r-img`, `cover-d-r-img__el`                                           | ✅ Live (GSAP; Phase 1B). Horizontal `clip-path: inset()` wipe + content scale/shift settle. Used by `Featured` images. No overlay divs — clip-path replaces the legacy approach.                                         |
...
| Loading screen                | `LoadingScreen.astro` — the "infinito" wordmark draws in on load (CSS stroke-dashoffset outline + fill fade), then the overlay fades out.                                                              | ✅ Done (#8, 2026-07-19). See §6.1.                                                                                                              |
...
| **Tier**               | **T1 — Must preserve**                                                                                                                      |
| **Modern replacement** | ✅ Done (#8, 2026-07-19). **CSS dasharray** (the §6.1-sanctioned alternative to DrawSVG), not GSAP. The uppercase INFINITO logotype (legacy glyph paths, `pathLength="100"`) draws its outline (`stroke-dashoffset` 100→0) then fades the fill in — a per-glyph CSS keyframe staggered 0.12 s via `--i`, running only under `html.has-anim` so reduced-motion / no-JS show the solid wordmark. `LoadingScreen.astro`'s script only holds the overlay until the last glyph's `animationend` **and** the window has loaded, then fades. **Why CSS, not DrawSVG:** the loader is above-the-fold, and the #43/#48 rule forbids gating first paint on the JS runtime. Every GSAP arrangement either regressed real (devtools) LCP +309 ms by evaluating GSAP before first paint (static import) or failed the simulate perf gate by lazily re-chunking GSAP (dynamic import); CSS-first matches main on both gates (devtools LCP 2830 ms = main; simulate perf 0.91, LCP 3079 ms = main). The infinity-symbol snake loop from the legacy is not reproduced — the rebuild keeps its own pulsing symbol mark. |
...
| **Tier**               | **T4 — Remove**                                                                                                                                                      |
| **Modern replacement** | Replace with the CSS intro keyframes above (fire at parse). Buyers who want a Pace-style bar can integrate **NProgress** (lightweight, MIT) — document it.      |
