...
> Last updated: **2026-07-09** — `cover-transp` per-line text reveal shipped (#9): `registerCoverTransp` uses GSAP SplitText `mask:"lines"` to split About + Featured body copy into lines that wipe up from inside a clip mask on scroll; the reusable line splitter for #10 / #64. Contract: `tests/e2e/cover-transp.spec.ts`. Build + 33 tests green; LHCI green (home perf 0.92, JS 118 KB gz). Baseline: `docs/baseline/cover-transp/`.
>
...
| 9   | `cover-transp` per-line text reveal | ✅ Done | 2026-07-09 | GitHub #9. `registerCoverTransp` — GSAP **SplitText** `type:"lines", mask:"lines"` splits the text; each line wipes up from inside its clip mask, staggered top→bottom on scroll (`gsap.from` yPercent:100, 0.6s, 0.1s stagger, power3.out). `autoSplit` re-splits on resize/font-load; `aria:"none"` keeps split spans in the a11y tree. Applied to `About` + `Featured` body copy (`data-anim="cover-transp"`). The reusable line splitter for #10 / #64. Contract: `tests/e2e/cover-transp.spec.ts`. Baseline: `docs/baseline/cover-transp/`. Build + 33 tests green; LHCI green (perf 0.92, JS 118 KB gz). |
