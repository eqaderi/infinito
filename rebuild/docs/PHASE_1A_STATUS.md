...
> Last updated: **2026-07-09** — `cover-transp` per-line text reveal shipped (#9): `registerCoverTransp` uses GSAP SplitText `mask:"lines"` to split About + Featured body copy into lines that wipe up from inside a clip mask on scroll; the reusable line splitter for #10 / #64. Contract: `tests/e2e/cover-transp.spec.ts`. Build + 34 e2e tests green; LHCI green (home perf 0.92, JS 118 KB gz). Baseline: `docs/baseline/cover-transp/`.
>
...
Deferred: `bars`, `slide-up2__lines` whole-block line-by-line splits (#10). `cover-transp` per-line reveals now live (#9).
...
- `slide-up2__lines` line-by-line split (#10) — the non-`cover-transp` variant still animates the whole block; reuse the #9 SplitText splitter for per-line motion.
