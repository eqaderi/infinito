...
> Last updated: **2026-07-19** — LoadingScreen wordmark draw-in shipped (#8): the uppercase INFINITO logotype draws its outline then fills via **CSS stroke-dashoffset** (not GSAP — the loader is above-the-fold, so #43/#48 forbids gating first paint on the JS runtime; §6.1 sanctions CSS dasharray). Measured all four routes — the two GSAP variants each failed one gate (static import regressed devtools LCP +309 ms; dynamic import failed the simulate perf gate via Vite re-chunking), CSS-first matches main on both (devtools LCP 2830 ms; simulate perf 0.91 / LCP 3079 ms). Contract: `tests/e2e/loading-screen.spec.ts`. Baseline: `docs/baseline/loading-screen/`. Build + 38 tests green. **Awaiting fidelity sign-off** (uppercase INFINITO ink-on-paper vs the legacy white-on-gradient wordmark).
>
> Prior: **2026-07-10** — `slide-up2__lines` per-line split completed (#10): the Team + Contact intro paragraphs — legacy `slide-up2__lines cover-transp`, which the rebuild still animated whole-block via `data-anim="slide-up"` — now use `data-anim="cover-transp"`, reusing the #9 SplitText splitter. In the legacy `slide-up2__lines` never occurs without `cover-transp` (scripts.js `.slide-up2` `cover_animation` + line-split at 5072), so this is a pure fidelity gain, no new primitive. Contract: `tests/e2e/cover-transp.spec.ts` (section-scoped `#team`/`#contact` cases). Build + 36 e2e tests green; LHCI green (home perf 0.92, LCP 3080 ms). Baseline: `docs/baseline/cover-transp/`.
>
> Prior: **2026-07-09** — `cover-transp` per-line text reveal shipped (#9): `registerCoverTransp` uses GSAP SplitText `mask:"lines"` to split About + Featured body copy into lines that wipe up from inside a clip mask on scroll; the reusable line splitter for #10 / #64. Contract: `tests/e2e/cover-transp.spec.ts`. Build + 34 e2e tests green; LHCI green (home perf 0.92, JS 118 KB gz). Baseline: `docs/baseline/cover-transp/`.
>
> Prior: LHCI perf budgets flipped warn→error (#79): `categories:performance` error at 0.90 (home median **0.92**) and `largest-contentful-paint` error at a **3400 ms** simulate ceiling (home median **3081 ms**; 3 runs 3080/3081/3083). AGENTS.md records 2.5 s as the real-throttle target; 3400 accommodates the documented Lantern text-LCP over-model. Gate green at error on the one built route (home).
...
- **GSAP 3 + ScrollTrigger** for scroll-driven reveals and parallax. GSAP went 100% free in April 2025, so the former Club plugins (**DrawSVG**, **SplitText**, MorphSVG) are now available — DrawSVG powers the Featured numeral draw-in; SplitText powers the `cover-transp` per-line text reveal (#9). MorphSVG remains unused for now.
...
| `LoadingScreen.astro` | Pulsing SVG infinity loop + the "infinito" wordmark, which **draws itself in** on load via DrawSVG (#8), then the overlay fades out.                                                                             |
...
Deferred: `bars`. `cover-transp` per-line reveals live (#9); `slide-up2__lines` per-line split done (#10 — Team + Contact intro copy switched from whole-block `slide-up` to the #9 splitter).
...
- ~~`cover-d-r-img` and `cover-up` primitives~~ — **done** (Phase 1B step 1, 2026-06-18).
- ~~`cover-transp` text line reveals~~ — **done** (#9, 2026-07-09). `registerCoverTransp` via SplitText `mask:"lines"`; the reusable line splitter for #10 / #64.
- ~~`slide-up2__lines` line-by-line split~~ — **done** (#10, 2026-07-10). Investigation found legacy `slide-up2__lines` never appears without `cover-transp` (same effect), so #10 was applying the #9 splitter to the two home-page intro paragraphs (Team + Contact) that were still whole-block `slide-up`. `data-anim="slide-up"` → `data-anim="cover-transp"`; no new primitive.
...
- ~~LoadingScreen wordmark draw-in~~ — **done** (#8, 2026-07-19). Uppercase INFINITO logotype (legacy glyph paths) draws its outline then fills via **CSS stroke-dashoffset** (not GSAP — above-the-fold LCP rule #43/#48; §6.1 sanctions CSS dasharray). Contract: `tests/e2e/loading-screen.spec.ts`. Baseline: `docs/baseline/loading-screen/`.
...
| 79  | Flip LCP + perf budgets warn→error | ✅ Done | 2026-07-09 | GitHub #79. `rebuild/lighthouserc.json`: `categories:performance` `warn`→`error` at `minScore 0.9` (home median 0.92 — a ~2-pt cushion over the floor); `largest-contentful-paint` `warn`→`error` with `maxNumericValue` `2500`→**`3400`** (home simulate median 3081 ms). Calibration (human-approved, AskUserQuestion): the committed `simulate` method over-models a **text** LCP ~1.1 s past visual completion (devtools cross-check = 1647 ms), so the enforced simulate ceiling is 3400 while AGENTS.md keeps 2.5 s as the real-throttle target. LHCI green at error. No source change. |
| 43  | Un-gate above-the-fold hero (LCP) | ✅ Done (budget → #79) | 2026-07-08 | GitHub #43. Hero intro (`intro-fade`/`intro-up`) moved off the GSAP `has-anim` opacity gate to **CSS keyframes that fire at parse** (`mountIntro` deleted); stagger via inline `--intro-delay`. `fonts.css` → `latin-*` subsets only (@font-face 77→14, `index.css` 118→58 KB). `x-cloak` on the nav drawer + search overlay (a pre-Alpine search-input placeholder was the phantom LCP element). Playfair-400 preload. (`inlineStylesheets: 'always'` was evaluated — ~150 ms FCP — but reverted: it didn't move LCP and zeroed the `stylesheet:size` LHCI assertion.) **Simulate LHCI:** perf 0.89→**0.92**, FCP 2405→2106 ms, LCP 3379→**3081 ms** (still >2500, kept `warn`). **Devtools throttling cross-check:** LCP **1647 ms**, perf **0.96** (observed paint 95 ms, SI 1956 ms). Residual LCP gap is Lantern over-modeling a text LCP ~1.1 s past visual completion — **budget/throttling recalibration handed to #79** (human-approved). Contract: `tests/e2e/hero-intro.spec.ts`. |
| 9   | `cover-transp` per-line text reveal | ✅ Done | 2026-07-09 | GitHub #9. `registerCoverTransp` — GSAP **SplitText** `type:"lines", mask:"lines"` splits the text; each line wipes up from inside its clip mask, staggered top→bottom on scroll (`gsap.from` yPercent:100, 0.6s, 0.1s stagger, power3.out). `autoSplit` re-splits on resize/font-load; `aria:"none"` keeps split spans in the a11y tree. Applied to `About` + `Featured` body copy (`data-anim="cover-transp"`). The reusable line splitter for #10 / #64. Contract: `tests/e2e/cover-transp.spec.ts`. Baseline: `docs/baseline/cover-transp/`. Build + 34 e2e tests green; LHCI green (perf 0.92, JS 118 KB gz). |
| 10  | `slide-up2__lines` per-line split | ✅ Done | 2026-07-10 | GitHub #10. Legacy `slide-up2__lines` never appears without `cover-transp` (scripts.js `.slide-up2` `cover_animation` + line-split at 5072) — same effect #9 built. So #10 = apply the #9 splitter to the two home-page intro paragraphs still animating whole-block: Team + Contact. `data-anim="slide-up"` → `data-anim="cover-transp"` (keeps `data-anim-delay="0.2"`, which `registerCoverTransp` honors). No new primitive; fidelity gain. Contract: section-scoped `#team`/`#contact` cases in `tests/e2e/cover-transp.spec.ts`. Build + 36 e2e tests green; LHCI green (perf 0.92, LCP 3080 ms). Baseline: `docs/baseline/cover-transp/` (effect signed off #9; `legacy-about-end.png` is the reference — legacy Team/Contact live capture is blocked by the preloader gating content on remote assets headless). |
| 8   | LoadingScreen wordmark draw-in | ✅ Done | 2026-07-19 | GitHub #8. The uppercase INFINITO logotype (legacy glyph paths, viewBox cropped to the glyph band, ink fill+stroke, `pathLength="100"`) draws its outline then fades the fill in via **CSS stroke-dashoffset** keyframes, staggered 0.12 s per glyph via `--i`, running only under `html.has-anim` (flash-free — `both` fill-mode holds the from-state through the stagger; reduced-motion / no-JS show the solid wordmark). `LoadingScreen.astro`'s script only holds the overlay until the last glyph's `animationend` **and** window load, then fades. **CSS, not DrawSVG** (as #8 first asked): the loader is above-the-fold, so gating first paint on GSAP violates #43/#48 and measurably regressed real LCP. Measured every route (main / static-GSAP / dynamic-GSAP / CSS): static import regressed devtools LCP +309 ms (GSAP eval before paint); dynamic import failed the simulate perf gate (Vite lazily re-chunks GSAP); **CSS-first matches main on both** — devtools LCP 2830 ms = main, simulate perf 0.91 / LCP 3079 ms = main. §6.1 lists CSS dasharray as the sanctioned alternative. Contract: `tests/e2e/loading-screen.spec.ts`. Baseline: `docs/baseline/loading-screen/`. Build + 38 tests green. |
| 48  | Codify above-the-fold rule in AGENTS.md | ✅ Done | 2026-07-09 | GitHub #48 (docs). Added the **Above-the-fold rule** to the Animation system section of AGENTS.md: first-viewport content is never JS-gated; the `has-anim` opacity FOUC gate is below-the-fold only (above-the-fold `[data-anim]` excepted to `opacity:1`, `intro-*` run CSS-first at parse); any above-the-fold GSAP entrance is visible-by-default; WebGL heroes render their poster in served HTML. Cross-links #43. Docs-only — no source, no test contract. |
...
5. ~~**LoadingScreen wordmark draw-in (#8)**~~ — **done** (2026-07-19); see §7.
