...
> Last updated: **2026-07-09** — LHCI perf budgets flipped warn→error (#79): `categories:performance` error at 0.90 (home 0.93) and `largest-contentful-paint` error at a **3400 ms** simulate ceiling (home 3079 ms). AGENTS.md records 2.5 s as the real-throttle target; 3400 accommodates the documented Lantern text-LCP over-model. Gate now green at error on every route.
...
| 79  | Flip LCP + perf budgets warn→error | ✅ Done | 2026-07-09 | GitHub #79. `rebuild/lighthouserc.json`: `categories:performance` `warn`→`error` at `minScore 0.9` (home 0.93); `largest-contentful-paint` `warn`→`error` with `maxNumericValue` `2500`→**`3400`** (home simulate 3079 ms). Calibration (human-approved, AskUserQuestion): the committed `simulate` method over-models a **text** LCP ~1.1 s past visual completion (devtools cross-check = 1647 ms), so the enforced simulate ceiling is 3400 while AGENTS.md keeps 2.5 s as the real-throttle target. LHCI green at error. No source change. |
